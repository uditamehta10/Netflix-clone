import React, { useEffect, useState } from 'react';
import axios from 'axios';
import './Navbar.css';
import {Link} from 'react-router-dom';
function Navbar(props){
  
    const [scroll,scrollbar]=useState(false);
        function changedvalue(event){
       props.getdata(event.target.value)
    }
    const [searchm,setsearchm]=useState('');
     function searched(event){
        setsearchm(event.target.value);
        props.getdata(searchm);
     }
     useEffect(()=>{},[searchm])

    useEffect(() => {
        window.addEventListener("scroll",()=>{
            if(window.scrollY>100){
            console.log("navcar");
             scrollbar(true);
            }else
            scrollbar(false);
        });
       
        
        
    }, []);
    
    return <div className={`navbar ${scroll && "navblack"}`}>
     <div className="logoul">
    <div>
    <Link exact to="/">  <img  className="navlogo" src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/Netflix_2015_logo.svg/1200px-Netflix_2015_logo.svg.png"/></Link>
    </div>
    <div>
        <ul className="navul">
        <li><Link exact to="/"style={{ color: 'white',textDecoration:'none' }}>Home</Link></li>
        <li><Link to="/trending"style={{ color: 'white',textDecoration:'none' }}>Trending</Link></li>
        <li><Link to="/about"style={{ color: 'white',textDecoration:'none' }}>About Us</Link></li>
        </ul>
    </div>
    
     </div> 
     <div>  
         <div className="flex">
            <input type="text" onChange={(event)=>{searched(event)}}/>
         <Link  to={`/searchmovie/${searchm}`}style={{ color: 'white',textDecoration:'none' }}><button  className="navbutton">Search...</button></Link>
         <button className="logsig" >Log In/Sign up</button>
         <img className="navimage" src="data:image/png;base64,"/>
         </div>
     </div> 
</div>

}export default Navbar;